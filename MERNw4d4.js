...
function containerWithMostWater(heights) {
    ...
    for (let i = 0; i < heights.length; i++) {
        ...
        while (runner < heights.length) {
            let edgeHeight = heights[i] > heights[runner] ? heights[runner] : heights[i];
            biggestContainer = edgeHeight * length > biggestContainer ? edgeHeight * length : biggestContainer;
            runner++
            length++
        }
    }
