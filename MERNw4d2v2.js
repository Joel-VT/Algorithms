...
function groupObjects(items) {
    ...
    for (let i = 0; i < items.length; i++) {
        if (expected.hasOwnProperty(items[i].category.toLowerCase())) {
            expected[items[i].category.toLowerCase()].push(items[i]);
        }
        else {
            expected[items[i].category.toLowerCase()] = [items[i]];
        }
    }
    return expected;
}
